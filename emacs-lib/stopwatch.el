...
(defvar stopwatch-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "c" 'stopwatch-clear)
    map)
...
(defun stopwatch-toggle ()
  ...
  (with-current-buffer stopwatch-buffer-name
    ...
    (if stopwatch-start
	  ...
	  (setq stopwatch-acc (stopwatch-delta))
          (setq stopwatch-start nil)
          (stopwatch-timer-stop))
      (setq stopwatch-start (current-time))
      (insert (stopwatch-human stopwatch-acc))
      (stopwatch-timer-start))
    (setq buffer-read-only t)))

(defun stopwatch-clear ()
  "Clear the stopwatch accumulator."
  (interactive)
  (with-current-buffer stopwatch-buffer-name
    (setq buffer-read-only nil)
    (erase-buffer)
    (insert (stopwatch-human '(0 0 0) t))
    (setq stopwatch-acc '(0 0 0))
    (when stopwatch-start
      (setq stopwatch-start nil)
      (stopwatch-timer-stop))
    (setq buffer-read-only nil)))
...
(defun stopwatch-delta ()
  ...
  (time-add (time-subtract (current-time) stopwatch-start) stopwatch-acc))
